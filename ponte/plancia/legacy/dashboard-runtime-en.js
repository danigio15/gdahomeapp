...
function cdApplianceVisual(a,size){
  ...
  if(media.kind==='image') {
    ...
    return '<img src="'+src+'" alt="'+cdEsc(a&&a.name||'Appliance')+'" style="display:block;width:'+size+'px;height:'+size+'px;max-width:100%;object-fit:contain" onerror="cdApplianceImageFallback(this,\''+type+'\','+size+')">';
  }
...
function cdApplMainCard(a) {
  ...
  if(powerValue) metrics.push('<div class="appl-primary"><span>⚡ Power</span><strong>'+cdEsc(powerValue)+'</strong></div>');
...
function renderApplianceSection(force) {
  ...
  setHtml('appl-consumption-list', list.length ? list.map(a => { const st = cdApplStatus(a), w = st.w || 0, nm = cdEsc(a.name || cdApplianceName(a.icon)); return `<div class="appl-cons-row"><div><div class="appl-wide-name">${nm}</div><div class="appl-cons-bar"><div class="appl-cons-fill" style="width:${Math.min(100, w / max * 100)}%"></div></div></div><div class="appl-pwr">${Math.round(w)} W</div></div>`; }).join('') : `<div class="appl-empty">No appliance configured</div>`);
  const autos = list.filter(a => Array.isArray(a.automations) && a.automations.length);
  setHtml('appl-grid-automations', autos.length ? autos.map(a => `<div class="appl-wide-card"><div class="appl-wide-name">🤖 ${cdEsc(a.name || cdApplianceName(a.icon))}</div><div class="appl-wide-cat">${cdEsc(a.automations.map(x => x.name || x.entity || x).join(' · '))}</div></div>`).join('') : `<div class="appl-empty">🤖 No automation configured</div>`);
}
...
function editorRenderAppliances(){
  ...
  const rows=list.map((a,i)=>{
    ...
    return '<div class="ed-row"><div class="ed-row-main"><div class="ed-row-new"><span style="display:inline-flex;vertical-align:middle;color:#0ea5e9;">'+cdApplianceIcon(a.icon,18)+'</span> '+cdEsc(a&&a.name?a.name:cdApplianceName(a.icon))+' <span style="color:var(--text-dim,#94a3b8);font-size:11px;font-weight:700;">· '+ents.length+' entities</span></div><div class="ed-row-old mono">'+cdEsc(ents.join(', '))+'</div></div><div class="ed-del" onclick="edApplEdit('+i+')" style="color:#0ea5e9;" title="Modifica">✏️</div><div class="ed-del" onclick="edApplDel('+i+')" title="Elimina">🗑️</div></div>';
  ...
  return '<div class="ed-intro">Add your appliances (washing machine, dishwasher, oven, microwave…). You can link <b>one or more entities</b> to each; the status (Running / Standby / Off) is derived from <b>power draw in Watts</b>, so it works even with <b>non-wifi</b> appliances via a smart plug.</div>'
    ...
    +'<input id="appl-thr" class="ed-input" type="number" value="'+cdEsc(editing&&ed.threshold_run!=null?ed.threshold_run:5)+'" placeholder="Running threshold (Watts)">'
...
function _avvRenderPopup(a, idx, force) {
    ...
    const ents = a.entities || (a.entity ? [a.entity] : []);
    const matches = ents.filter(en => { const s = STATES[en]; return s && cdAvvisoActive(a, s); });
    document.getElementById('details-title').innerHTML = cdEsc(a.icon || '⚠️') + ' ' + cdEsc(String(a.name || 'Alert').toUpperCase());
...
function buildReportSelect() {
    ...
    sel.innerHTML = `<option value="">${runtimeError ? '⚠️ ' + cdEsc(runtimeError) : '— No Report items configured —'}</option>`;
}
...
function editorRenderAvvisi() {
        ...
        (GRUPPI_MONITORAGGIO[grp] || []).forEach(id => {
            ...
            rows += `<div class="ed-row">
              <div class="ed-row-main">
                ...
                <div class="ed-row-old mono">${glbl} · ${cdEsc(id)}</div>
              </div>
    ...
    if (customList.length) {
        const cinner = customList.map((a, i) => {
            const ents = a.entities || (a.entity ? [a.entity] : []);
            return `<div class="ed-row"><div class="ed-row-main"><div class="ed-row-new">${cdEsc(a.icon||'⚠️')} ${cdEsc(a.name||ents[0]||'')} <span style="color:var(--text-dim,#94a3b8); font-size:11px; font-weight:700;">· ${cdEsc(cdAvvisoCondLabel(a))}${ents.length>1?' · '+ents.length+' entities':''}</span></div><div class="ed-row-old mono">${cdEsc(ents.join(', '))}</div></div><div class="ed-del" onclick="edEditAvvisoCustom(${i})" style="color:#0ea5e9;" title="Edit">✏️</div><div class="ed-del" onclick="edDelAvvisoCustom(${i})" title="Delete">🗑️</div></div>`;
        }).join('');
...
function wzRender() {
    ...
    if (WIZ.step === 1) body = `
        ...
        <textarea id="wz-token" class="wz-input" rows="4" placeholder="Paste your token here (starts with eyJ...)">${cdEsc(WIZ.token)}</textarea>
        <input id="wz-remote" class="wz-input mono" style="font-family:monospace; font-size:12px;" placeholder="Optional remote URL (e.g. https://xxxx.ui.nabu.casa)" value="${cdEsc(WIZ.remoteUrl||'')}">
    ...
    if (WIZ.step === 2) body = `
        ...
        <input id="wz-title-in" class="wz-input" value="${cdEsc(WIZ.title)}" placeholder="My Home">
    ...
    if (WIZ.step === 4) {
        ...
        <input class="wz-input" style="margin-bottom:8px;" placeholder="🔍 Search light… (name or entity)" value="${cdEsc(WIZ.lightsFilter||'')}" oninput="WIZ.lightsFilter = this.value.trim().toLowerCase(); wzRenderLights();">
    ...
    if (WIZ.step === 5) {
        ...
        <details class="ed-acc"><summary class="ed-acc-head">⚡ Quick actions <span class="ed-acc-n">${(WIZ.quickActions||[]).length}</span></summary>
          <div class="ed-acc-body">
            ${(WIZ.quickActions||[]).map((a, i) => `<div class="ed-row"><div style="font-size:15px;">${cdEsc(a.icon||'⚡')}</div><div class="ed-row-main"><div class="ed-row-new">${cdEsc(a.name||a.builtin||'?')}</div><div class="ed-row-old mono">${a.type === 'luci_group' ? '💡 ' + ((a.lights||[]).length) + ' lights' : cdEsc(a.type)}</div></div>${a.type === 'luci_group' ? `<div class="ed-del" style="background:rgba(14,165,233,0.12);" onclick="wzEditLightGroup(${i})">✏️</div>` : ''}<div class="ed-del" onclick="WIZ.quickActions.splice(${i},1); wzRender();">🗑️</div></div>`).join('')}
        ...
        <details class="ed-acc"><summary class="ed-acc-head">🔔 Alerts Panel <span class="ed-acc-n">${(GRUPPI_MONITORAGGIO['win']||[]).length + (GRUPPI_MONITORAGGIO['batt']||[]).length + cdCfgList('cd_avvisi_custom').length}</span></summary>
          <div class="ed-acc-body">
            ...
            ${['win','batt'].map(grp => (GRUPPI_MONITORAGGIO[grp]||[]).map(id => {
                ...
                return `<div class="ed-row"><div style="font-size:15px;">${grp==='win'?'🚪':'🔋'}</div><div class="ed-row-main"><div class="ed-row-new">${cdEsc(nm)}</div><div class="ed-row-old mono">${cdEsc(id)}</div></div>${grp==='win'?`<div class="ed-del" title="Flip the direction (ON = closed)" style="${dmVersoInvertito(id)?'background:linear-gradient(135deg,#0ea5e9,#0369a1);color:#fff;':''}" onclick="edAvvVerso(${cdJs(id)})">⇄</div>`:''}<div class="ed-del" onclick="wzDelAvviso(${cdJs(grp)},${cdJs(id)})">🗑️</div></div>`;
            }).join('')).join('')}
...
function wzTestToken() {
    ...
    try {
        ...
        tws.onerror = () => { if (!done) res.innerHTML = '<div class="wz-err">❌ Cannot reach HA at ' + cdEsc(_ha.base || location.host) + '.<br>Check the URL and that HA is reachable from this device. If the file was opened from a download, the URL above is required.</div>'; };
...
function cdPickLights(groupName, onDone, preselected) {
    ...
    ov.innerHTML = `<div style="background:var(--card-bg,#fff); border-radius:22px; padding:20px; width:min(430px,100%); max-height:80vh; display:flex; flex-direction:column; box-shadow:0 20px 60px rgba(0,0,0,0.3);">
        <div style="font-weight:900; font-size:15px; margin-bottom:4px; color:var(--text,#0f172a);">💡 Lights in group "${cdEsc(groupName)}"</div>
...
const CD_SLOTS = {
    ...
    ev: { label: '🚗 Electric car', slots: [
        ...
        { ref: 'dm.ev_ricarica_sempre_evcc', lbl: 'EVCC always charge (select)' },
    ...
    const sezAccordions = allSections.map(([key, sec]) => {
        const rows = sec.slots.map(sl => {
            ...
            return `<div class="ed-slot">
              <div class="ed-slot-lbl"><input class="wz-lbl-edit" value="${cdEsc(lbl)}" onchange="wzRenameSlot(${cdJs(sl.ref)}, this.value)" title="Tap to rename the label"></div>
    ...
    return `
      ...
      <details class="ed-acc"><summary class="ed-acc-head">🌡️ Temperatura <span class="ed-acc-n">${stanze.length} rooms</span></summary>
        ...
        <div class="ed-acc-body">
          <div class="ed-list">${getQuickActions().map((a, i) => {
            ...
            return `<div class="ed-row">
              ...
              <div class="ed-row-main">
                ...
                <div class="ed-row-old mono">${cdEsc(a.type === 'builtin' ? 'popup nativo: ' + a.builtin : (a.type === 'luci_group' ? '💡 gruppo · ' + ((a.lights||[]).length) + ' lights' : a.type + ' · ' + (a.entity||'') + (a.option ? ' → ' + a.option : '')))}</div>
              </div>
          ...
          <div class="ed-form">
            ...
            <div id="ed-qa-option-row" style="display:none; gap:6px; align-items:center;"><span style="font-size:11px; color:var(--text-dim); flex:0 0 auto;">Which option</span><select id="ed-qa-option" class="ed-input" style="flex:1;"></select></div>
      ...
      <details class="ed-acc"><summary class="ed-acc-head">📊 Report Analisi <span class="ed-acc-n">${cdCfgList('cd_report_devices').length} entries</span></summary>
        <div class="ed-acc-body">
          ...
          ${cdCfgList('cd_report_devices').map((d, ri) => `<div class="ed-row"><div style="font-size:15px;">${cdEsc(d.icon||'⚡')}</div><div class="ed-row-main"><div class="ed-row-new">${cdEsc(d.name)}</div><div class="ed-row-old mono">${cdEsc(d.entity)}</div></div><div class="ed-del" onclick="edDelReportDevice(${ri})">🗑️</div></div>`).join('') || '<div class="ed-empty">No entries</div>'}
          <div style="display:flex; gap:8px; margin-bottom:8px;">
...
function edQaEntityChanged() {
    ...
    sel.innerHTML = '<option value="">— chosen each time, from the popup —</option>' + voci.map(v => `<option value="${esc(v)}">${esc(v)}</option>`).join('');
    if (voci.includes(prima)) sel.value = prima;
    row.style.display = 'flex';
    if (hint) hint.textContent = '🎚️ This entity is a dropdown: the button opens a popup with its options. Or fix one here.';
}
...
function apriGestioneLuci(isClick = false, luciFilter = null, customTitle = null) {
  ...
  document.getElementById('details-title').innerHTML = window._luciPopupTitle ? ('💡 ' + cdEsc(window._luciPopupTitle.toUpperCase())) : '💡 LIGHTS';
...
function cdShowUpdateOverlay(target) {
    ...
    o.innerHTML = '<div style="width:46px; height:46px; border:4px solid ' + (dark ? 'rgba(56,189,248,0.22)' : 'rgba(14,165,233,0.22)') + '; border-top-color:#0ea5e9; border-radius:50%; animation:cdSpin 0.8s linear infinite;"></div><div style="font-size:15px; font-weight:800; letter-spacing:1px; color:' + (dark ? '#7dd3fc' : '#0369a1') + '; text-transform:uppercase;">Updating to v' + cdEsc(target) + '\u2026</div>';
...
async function dmCamOpen(cam, title, content) {
    ...
    if (camState === 'unavailable' || camState === 'unknown') {
        content.innerHTML = `<div class="cam-popup-error">⚠️ Camera offline<br><span style="font-weight:500;opacity:.85;font-size:12px;text-transform:none;letter-spacing:0;">${cdEsc(cam.entity)} not responding (state: ${cdEsc(camState || 'unknown')})</span></div>`;
        return;
    }
    ...
    content.innerHTML = '<div class="cam-popup-error">⚠️ Unable to open the camera<br><span style="font-weight:500;opacity:.85;font-size:12px;text-transform:none;letter-spacing:0;">' + righe.map(function (riga) { return cdEsc(riga); }).join('<br>') + '</span></div>';
}
...
async function dmCamMJPEG(cam, content, attesa) {
    ...
    content.innerHTML = `<div class="cam-popup-body"><div id="video-iframe-container" class="cam-zoom-container" style="position:relative; padding-top:56.25%;"><img id="cam-mjpeg" src="${cdUrl(streamUrl)}" alt="${cdEsc(cam.entity)}"><div id="cam-video-loader" class="cam-video-loader-overlay"><div class="cam-popup-spinner"></div><div>Connecting stream…</div></div>${_DM_FS_BTNS}</div><button id="toggle-fs-main-btn" class="cam-fs-btn" onclick="toggleFullScreenCam()">🔲 Fullscreen</button><div class="cam-popup-hint"><span class="cam-mode-badge mjpeg">MJPEG</span> Continuous live stream</div></div>`;
...
async function dmCamPolling(cam, content) {
    ...
    if (!buildUrl()) { content.innerHTML = `<div class="cam-popup-error">⚠️ Stream unavailable<br><span style="font-weight:500;opacity:.85;font-size:12px;text-transform:none;letter-spacing:0;">Camera ${cdEsc(cam.entity)} does not expose snapshots</span></div>`; return; }
...
window.dmAttivaAudio = async function () {
    ...
    catch (e) { content.innerHTML = `<div class="cam-popup-error">⚠️ Audio unavailable<br><span style="font-weight:500;opacity:.85;font-size:12px;text-transform:none;letter-spacing:0;">${cdEsc(e.message || e)}</span></div>`; setTimeout(() => { const c = document.getElementById('details-list'); if (c) dmCamPolling(cam, c); }, 3500); }
};
          ...
          if (m.success && m.result && m.result.response && m.result.response[entityId]) {
              ...
              pendingWsCallbacks[reqId2] = (m2) => {
                  if (m2.success && m2.result && m2.result.response && m2.result.response[entityId]) {
                       ...
                       var nfRadar = document.querySelector('.dm-radar-blocco') ? ' The radar above is a separate thing and works on its own.' : ''; list.innerHTML = `<div style="text-align:center; padding:20px; color:var(--text-dim,#64748b); line-height:1.5;"><strong style="display:block; font-size:14px; font-weight:800; color:var(--text,#0f172a); margin-bottom:6px;">This weather entity has no forecast</strong>It reports the weather right now, not the next days. To see the seven days pick another weather entity under Config › Sections › Weather.${nfRadar}<code style="display:block; margin-top:10px; font-size:11px; color:var(--text-dim,#64748b);">${cdEsc(entityId)}</code></div>`;
                  }
              };
...
function renderForecasts(forecasts) {
  const list = document.getElementById('weather-forecast-list');
  if(!forecasts || forecasts.length === 0) { var nfRadar = document.querySelector('.dm-radar-blocco') ? ' The radar above is a separate thing and works on its own.' : ''; list.innerHTML = `<div style="text-align:center; padding:20px; color:var(--text-dim,#64748b); line-height:1.5;"><strong style="display:block; font-size:14px; font-weight:800; color:var(--text,#0f172a); margin-bottom:6px;">This weather entity has no forecast</strong>It reports the weather right now, not the next days. To see the seven days pick another weather entity under Config › Sections › Weather.${nfRadar}<code style="display:block; margin-top:10px; font-size:11px; color:var(--text-dim,#64748b);">${cdEsc(currentWeatherEntity || '')}</code></div>`; return; }
...
async function apriSrvHistory(tipo) {
    ...
    try {
        ...
        timeline.innerHTML = `<div style="text-align:center;padding:24px;color:#ef4444;font-weight:800;font-size:12px;">❌ History non disponibile<br><span style="color:var(--text-dim);font-size:10px;">${cdEsc(e.message)}</span></div>`;
    }
}
