 ...
 *   GET    /console/casa/<casa_…>/plancia/<profilo>   com'e' fatta quella plancia
 *   PUT    /console/casa/<casa_…>/plancia/<profilo>   scrivila cosi', al prossimo rapporto
 *
...
export function costruisciIlServer({
  ...
  async function servi(richiesta, risposta) {
    ...
    if (via === "/plancia" && metodo === "POST") {
      ...
      const preso = plance.prendi(casa, {
    ...
    if (ritiro && metodo === "GET") {
      ...
      const chiesta = plance.daConsegnare(casa, ritiro[1], id);
      if (!chiesta) {
    ...
    if (via === "/rapporto" && metodo === "POST") {
      ...
      const vuoleLaPlancia =
        carta?.configurazione === true ? plance.quali(casa, carta?.plance?.elenco) : [];
      json(risposta, {
        presa: true,
        di: suo?.nome || "",
  ...
  async function ilRetro(richiesta, risposta, via, metodo, chi) {
     ...
     * gia' in coda. */
    const plancia = /^\/casa\/(casa_[0-9a-f]{32})\/plancia\/([a-z0-9][a-z0-9-]{0,63})$/.exec(via);
    if (plancia && (metodo === "GET" || metodo === "PUT")) {
      ...
      if (metodo === "GET") {
        json(risposta, { scatto: plance.scatto(plancia[1], plancia[2]) });
        return;
      }
      let detto;
      try {
      ...
      plance.chiedi(plancia[1], plancia[2], { id: messo.id, valori, revisioneAttesa });
    ...
    if (lavoro && metodo === "DELETE") {
      ...
      if (annullato && inCoda?.cosa === "configura")
        plance.dimenticaLaChiesta(lavoro[1], inCoda.nome);
